...
ei + geom_point()+ geom_smooth(method = "lm") 

fi + geom_point()+ geom_smooth(method = "lm")

fe + geom_point()+ geom_smooth(method = "lm")

dict <- read_lines(file.choose(), skip = 18)

labels_raw <- read_file(file.choose())
