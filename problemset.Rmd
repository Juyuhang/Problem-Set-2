...
The data used to study this relationship was obtained from the Canadian General Social Survey of 2017. The data was made from an interview or a questionnaire which were filled by individuals about themselves. This dataset contained a large amount of data about various aspects of family. In essence, the dataset had 20,602 rows of data divided into 80 categories. These categories included the respondents? age, occupation, educational level, incomes, and more information about themselves, their partners, and their children. The respondents were residents in Canada aged between 15 and 80 years of age. This means that each one of them could provide reliable information about himself or herself. The dataset contained both qualitative and quantitative data since it was created to be as exhaustive as possible. It was then coded into .csv format in order to summarize it in concisely. The information was coded appropriately in order to facilitate easy and effective analysis of the data. However, the data was limited to the Canadian demographic. It also lacked the aspect of change with time and was only useful for a cross section study of the variables of age at first birth, personal income, and education. The change in the variables with time is crucial to identify the progress the country is making towards supporting women as an effort towards achieving gender equality in all matters. Further, the respondents were provided with choices in giving their responses, while this increased the efficiency for the research process, it made the analysis less effective. This is because the data appeared clustered although it was not actually clustered in real life. 

##Model

In order to analyze this data, this study isolated these variables and formatted them in such a way to facilitate the creation of a linear model. After importing the dataset into R as a .csv file, the age at first birth dataset, named ```{r} raw_data$achb1c``` in the original file, was changed from a factor into a numeric to facilitate the creation of a model. The NAs were stripped from the data before defining the linear model. 
A linear model was chosen due to the direct relationships between the three variables. Furthermore, the hypotheses made by the study before fitting the model. The data was fitted to the linear model summarised in the table below. In order to understand the relationship between the independent variables, the study also included separate linear models which represented the relationship between education and personal income, and between age at first birth and personal income. 
